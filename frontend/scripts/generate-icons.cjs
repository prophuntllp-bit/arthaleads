...
async function main() {
  ...
  // Write to a temp file first, then rename over logo.png to avoid file-lock issues
  const srcBuffer = fs.readFileSync(SRC);
  const f512 = roundedMask(512);
  const logoTmp = path.join(PUB, "logo.tmp.png");
  await sharp(srcBuffer)
    .resize(512, 512, { fit: "cover" })
    .composite([{ input: f512, blend: "dest-in" }])
    .png()
    .toFile(logoTmp);
  fs.renameSync(logoTmp, path.join(PUB, "logo.png"));
