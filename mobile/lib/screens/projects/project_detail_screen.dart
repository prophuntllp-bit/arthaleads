...
class _ProjectDetailScreenState extends State<ProjectDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _editProject() async {
    ...
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(builder: (_) => ProjectFormScreen(project: _project, agents: agents)),
    );
    if (saved == true) {
      final fresh = await ApiClient.instance.dio.get('/projects/$_projectId');
      if (mounted) setState(() => _project = (fresh.data['data'] as Map).cast<String, dynamic>());
    }
  }
  ...
  Future<void> _deleteProject() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('"${_project['name']}" will be archived.'),
        actions: [
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            itemBuilder: (ctx) => const [
              ...
              PopupMenuItem(value: 'delete', child: Text('Archive Project')),
            ],
          ),
        ],
        bottom: TabBar(
          controller: _tabController,
          tabs: const [Tab(text: 'Info'), Tab(text: 'Leads'), Tab(text: 'Prospective')],
        ),
      ),
      body: TabBarView(
        controller: _tabController,
    ...
    return ListView(
      ...
      children: [
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, i) => ClipRRect(
                ...
                child: Image.network(images[i], width: 200, fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => Container(width: 200, color: Colors.grey.shade800)),
              ),
            ),
          ),
        ...
        if (bhk.isNotEmpty) ...[
          ...
          const SizedBox(height: 4),
          Wrap(spacing: 6, children: bhk.map((b) => Chip(label: Text(b))).toList()),
        ],
        if (amenities.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text('Amenities', style: Theme.of(context).textTheme.labelLarge),
          const SizedBox(height: 4),
          Wrap(spacing: 6, runSpacing: 4, children: amenities.map((a) => Chip(label: Text(a))).toList()),
        ],
        if (assigned.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text('Assigned Agents', style: Theme.of(context).textTheme.labelLarge),
          const SizedBox(height: 4),
          Text(assigned.join(', ')),
        ],
      ],
...
class _LeadsTabState extends State<_LeadsTab> {
  ...
  Future<void> _bulkDelete() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('${_selected.length} lead(s) will be permanently deleted.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
    ...
    if (ok != true) return;
    try {
      await _api.dio.delete('/projects/$_projectId/leads/bulk', data: {'ids': _selected.toList()});
      _load(reset: true);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  Future<void> _bulkStatus() async {
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: bookingOptions
              ...
              .map((o) => ListTile(title: Text(o.label), onTap: () => Navigator.pop(ctx, o.value)))
              .toList(),
        ),
      ),
    );
    ...
    try {
      await _api.dio.patch('/projects/$_projectId/leads/bulk-status', data: {'ids': _selected.toList(), 'booking': booking});
      _load(reset: true);
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(ApiClient.errorMessage(e, 'Bulk status update failed')),
          backgroundColor: AppColors.danger,
        ));
      }
    }
  ...
  Future<void> _importCsv() async {
    try {
      ...
      final dataRows = rows.skip(1).map((r) => {
            ...
            if (emailIdx != -1 && emailIdx < r.length) 'email': r[emailIdx].toString().trim(),
          }).where((r) => (r['name'] ?? '').toString().isNotEmpty && (r['phone'] ?? '').toString().isNotEmpty).toList();
      if (dataRows.isEmpty) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No valid rows found')));
        return;
      }
      final res = await _api.dio.post('/projects/$_projectId/leads/import', data: {'rows': dataRows});
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Imported ${res.data['imported'] ?? dataRows.length}, '
  ...
  Future<void> _exportCsv() async {
    final rows = [
      ...
      ..._leads.map((l) => [l['name'] ?? '', l['phone'] ?? '', l['email'] ?? '', l['booking'] ?? '', l['status'] ?? '']),
    ];
    final csv = const ListToCsvConverter().convert(rows);
    await Share.shareXFiles([
      XFile.fromData(Uint8List.fromList(utf8.encode(csv)), name: '${widget.project['name']}_leads.csv', mimeType: 'text/csv'),
    ]);
  }
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(hintText: 'Search leads…', prefixIcon: Icon(Icons.search, size: 20), isDense: true),
                    textInputAction: TextInputAction.search,
                    onSubmitted: (_) => _load(reset: true),
                  ),
                ),
                IconButton(icon: const Icon(Icons.upload_file, size: 20), tooltip: 'Import CSV', onPressed: _importCsv),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                if (_selectMode)
                  TextButton(onPressed: () => setState(() => _selected.clear()), child: Text('Clear (${_selected.length})')),
              ],
            ),
          ),
          Expanded(
                                ...
                                subtitle: Column(
                                  ...
                                  children: [
                                    ...
                                    if ((lead['booking'] as String? ?? '').isNotEmpty)
                                      Padding(padding: const EdgeInsets.only(top: 4), child: BookingChip(lead['booking'] as String?)),
                                  ],
                                ),
                                trailing: Row(
                                  mainAxisSize: MainAxisSize.min,
