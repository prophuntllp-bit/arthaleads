...
class _PipelineScreenState extends State<PipelineScreen> {
  ...
  Future<void> _markContacted(Map<String, dynamic> lead) async {
    ...
    } catch (_) {/* silent — the WhatsApp chat still opened */}
  }
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          const SizedBox(width: 8),
                                          InkWell(
                                            onTap: () => _whatsapp(lead),
                                            child: const Icon(Icons.chat, size: 16, color: AppColors.whatsapp),
                                          ),
                                        ],
                                      ),
                                      ...
                                      if (((lead['followUpDate'] ?? lead['followUp']) as String? ?? '').isNotEmpty) ...[
                                        const SizedBox(height: 6),
                                        Row(
                                          children: [
                                            ...
                                            Expanded(
                                              child: Text(
                                                DateFormat('dd MMM, hh:mm a').format(
                                                  DateTime.parse((lead['followUpDate'] ?? lead['followUp']) as String).toLocal(),
                                                ),
                                      ...
                                      if ((lead['remark'] as String? ?? '').isNotEmpty ||
                                          (lead['remark1'] as String? ?? '').isNotEmpty) ...[
                                        const SizedBox(height: 4),
                                        Text(
                                          (lead['remark'] as String? ?? '').isNotEmpty
