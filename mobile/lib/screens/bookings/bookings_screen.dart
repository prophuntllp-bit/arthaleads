...
class _BookingsScreenState extends State<BookingsScreen> {
  ...
  Future<void> _openForm({Map<String, dynamic>? booking}) async {
    ...
    final jointBuyerCtrl = TextEditingController(text: booking?['jointBuyerName'] as String? ?? '');
    final projectCtrl = TextEditingController(text: booking?['projectName'] as String? ?? '');
    final phaseCtrl = TextEditingController(text: booking?['phase'] as String? ?? '');
    final unitCtrl = TextEditingController(text: booking?['unitNo'] as String? ?? '');
    final towerCtrl = TextEditingController(text: booking?['tower'] as String? ?? '');
    final valueCtrl = TextEditingController(text: booking?['considerationValue']?.toString() ?? '');
    final brokeragePctCtrl = TextEditingController(text: booking?['brokeragePercent']?.toString() ?? '2');
    final brokerageAmtCtrl = TextEditingController(text: booking?['brokerageAmount']?.toString() ?? '');
    final adjustmentCtrl = TextEditingController(text: booking?['brokerageAdjustment']?.toString() ?? '0');
    final fosCtrl = TextEditingController(text: booking?['fosIncentive']?.toString() ?? '0');
    final eoiCtrl = TextEditingController(text: booking?['eoiIncentive']?.toString() ?? '0');
    final notesCtrl = TextEditingController(text: booking?['notes'] as String? ?? '');
    ...
    DateTime bookingDate = DateTime.tryParse(booking?['bookingDate']?.toString() ?? '') ?? DateTime.now();
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSheetState) {
          void applyDeveloperDefaults(String? devId) {
            ...
            final dev = _developers.firstWhere((d) => d['_id'] == devId, orElse: () => {});
            if (dev.isEmpty) return;
            setSheetState(() {
              brokeragePctCtrl.text = (dev['defaultBrokeragePercent'] ?? 2).toString();
          ...
          final brok = manualBrokerage ? parseNum(brokerageAmtCtrl.text) : (cv * pct / 100);
          ...
          Widget calcRow(String label, String value, {bool bold = false, Color? color}) => Padding(
                padding: const EdgeInsets.symmetric(vertical: 2),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(label, style: TextStyle(fontSize: 11, color: bold ? null : Colors.grey, fontWeight: bold ? FontWeight.w700 : FontWeight.normal)),
                    Text(value, style: TextStyle(fontSize: 11, fontWeight: bold ? FontWeight.w800 : FontWeight.w600, color: color)),
                  ],
                ),
              );
          ...
          return Padding(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text(editing ? 'Edit Booking' : 'New Booking', style: Theme.of(ctx).textTheme.titleLarge),
                  const SizedBox(height: 16),
                  const Text('Customer Details', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
                  const SizedBox(height: 8),
                  TextField(controller: customerCtrl, decoration: const InputDecoration(labelText: 'Customer name *')),
                  const SizedBox(height: 12),
                  TextField(controller: jointBuyerCtrl, decoration: const InputDecoration(labelText: 'Joint buyer (optional)')),
                  const SizedBox(height: 16),
                  const Text('Unit Details', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
                  const SizedBox(height: 8),
                  DropdownButtonFormField<String>(
                    ...
                    items: _developers
                        ...
                        .toList(),
                    onChanged: editing ? null : (v) { developerId = v; applyDeveloperDefaults(v); },
                  ),
                  const SizedBox(height: 12),
                  ...
                  Row(
                    children: [
                      Expanded(child: TextField(controller: phaseCtrl, decoration: const InputDecoration(labelText: 'Phase'))),
                  ...
                  Row(
                    children: [
                      Expanded(child: TextField(controller: unitCtrl, decoration: const InputDecoration(labelText: 'Unit / Plot no. *'))),
                      const SizedBox(width: 12),
                      Expanded(child: TextField(controller: towerCtrl, decoration: const InputDecoration(labelText: 'Tower / Block'))),
                    ],
                  ),
                  const SizedBox(height: 12),
                  InkWell(
                    onTap: () async {
                      final picked = await showDatePicker(
                        context: ctx, initialDate: bookingDate,
                        firstDate: DateTime(2015), lastDate: DateTime(2100),
                      );
                      if (picked != null) setSheetState(() => bookingDate = picked);
                    },
                  ...
                  const SizedBox(height: 16),
                  const Text('Brokerage Details', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
                  const SizedBox(height: 8),
                  TextField(
                      ...
                      Expanded(
                        child: manualBrokerage
                            ? TextField(
                                ...
                                decoration: const InputDecoration(labelText: 'Brokerage amount (₹)'),
                                keyboardType: TextInputType.number,
                                onChanged: (_) => setSheetState(() {}),
                              )
                            : TextField(
                      ...
                      Column(
                        ...
                        children: [
                          Switch(
                            ...
                            onChanged: (v) => setSheetState(() => manualBrokerage = v),
                          ),
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Brokerage adjustment (-) (₹)'),
                    keyboardType: TextInputType.number,
                    onChanged: (_) => setSheetState(() {}),
                  ),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Expanded(
                        child: TextField(
                          controller: fosCtrl,
                          decoration: const InputDecoration(labelText: 'FOS incentive (₹)'),
                          keyboardType: TextInputType.number,
                          onChanged: (_) => setSheetState(() {}),
                        ),
                      ),
                      ...
                      Expanded(
                        child: TextField(
                          controller: eoiCtrl,
                          decoration: const InputDecoration(labelText: 'EOI incentive (₹)'),
                          keyboardType: TextInputType.number,
                          onChanged: (_) => setSheetState(() {}),
                        ),
                      ),
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton(
                          onPressed: () => setSheetState(() => gstType = 'CGST_SGST'),
                          style: OutlinedButton.styleFrom(
                            backgroundColor: gstType == 'CGST_SGST' ? AppColors.primary : null,
                            foregroundColor: gstType == 'CGST_SGST' ? Colors.white : null,
                          ),
                          child: const Text('CGST + SGST', style: TextStyle(fontSize: 12)),
                        ),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: OutlinedButton(
                          onPressed: () => setSheetState(() => gstType = 'IGST'),
                          style: OutlinedButton.styleFrom(
                            backgroundColor: gstType == 'IGST' ? AppColors.primary : null,
                            foregroundColor: gstType == 'IGST' ? Colors.white : null,
                          ),
                          child: const Text('IGST (Interstate)', style: TextStyle(fontSize: 12)),
                        ),
                      ),
                    ],
                  ),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        const Text('LIVE CALCULATION', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
                        const SizedBox(height: 6),
                        calcRow('Brokerage Amount', _fmtMoney(brok)),
                        if (adj > 0) calcRow('Adjustment (-)', '-${_fmtMoney(adj)}', color: AppColors.danger),
                        if (fos > 0) calcRow('FOS Incentive', _fmtMoney(fos), color: const Color(0xFF6366F1)),
                        if (eoi > 0) calcRow('EOI Incentive', _fmtMoney(eoi), color: AppColors.success),
                        calcRow('Total Brokerage', _fmtMoney(total), bold: true),
                  ...
                  GradientButton(
                    ...
                    onPressed: () async {
                      ...
                      final data = {
                        ...
                        if (manualBrokerage) 'brokerageAmount': parseNum(brokerageAmtCtrl.text),
  ...
  Widget build(BuildContext context) {
    final totalBill = _bookings.fold<num>(0, (s, b) => s + ((b['totalBill'] as num?) ?? 0));
              ...
              child: ListView(
                ...
                children: [
                  ...
                  GridView.count(
                    ...
                    children: [
                      _statCard('Total Bookings', '${_bookings.length}', const Color(0xFF6366F1)),
                      _statCard('Total Brokerage', _fmtMoney(totalBill), AppColors.primary),
                      _statCard('Invoice Pending', '$countPending', AppColors.warning),
                      _statCard('Payment Received', '$countPaid', AppColors.success),
                    ],
                  ),
                  const SizedBox(height: 12),
                  // ── Filter tabs ──
                  SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        for (final f in [['all', 'All'], ['new', 'New'], ['invoiced', 'Invoiced'], ['payment_received', 'Paid']])
                          Padding(
                            padding: const EdgeInsets.only(right: 6),
                            child: ChoiceChip(
                              label: Text(f[1], style: const TextStyle(fontSize: 11.5)),
  ...
  Widget _bookingCard(Map<String, dynamic> b) {
    ...
    final dev = b['developerId'] is Map ? (b['developerId'] as Map)['name'] as String? : null;
    final unitBits = [
      b['unitType'],
      b['unitNo'],
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(b['customerName'] as String? ?? '—', style: const TextStyle(fontWeight: FontWeight.w600)),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if ((b['jointBuyerName'] as String? ?? '').isNotEmpty)
              Text(b['jointBuyerName'] as String, style: const TextStyle(fontSize: 11, color: Colors.grey)),
            Text('${b['projectName'] ?? ''} · $unitBits'),
            if (dev != null) Text(dev, style: const TextStyle(fontSize: 11, color: Colors.grey)),
            Text(_fmtDate(b['bookingDate']), style: const TextStyle(fontSize: 11, color: Colors.grey)),
            ...
            Row(
              children: [
                ...
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(_fmtMoney(b['totalBill']), style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
                    Text('Brok: ${_fmtMoney(b['totalBrokerage'])}', style: const TextStyle(fontSize: 10, color: Colors.grey)),
                  ],
                ),
              ],
            ),
        ...
        trailing: PopupMenuButton<String>(
          ...
          itemBuilder: (ctx) => [
            ...
            if (b['invoiceId'] != null)
              const PopupMenuItem(value: 'view_invoice_note', enabled: false, child: Text('View invoice on web app')),
