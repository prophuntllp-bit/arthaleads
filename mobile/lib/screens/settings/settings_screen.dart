...
/// of frontend/src/pages/Settings.jsx; EnableX telephony config stays web-only.
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  final Map<String, TextEditingController> _billing = {
    for (final k in ['address', 'phone', 'email', 'gstNo', 'pan', 'cin', 'rera',
  ...
  Future<String?> _pickImageAsDataUri() async {
    final photo = await ImagePicker().pickImage(source: ImageSource.gallery, imageQuality: 75, maxWidth: 800);
  ...
  Future<void> _saveProfile() async {
    if (_newPassword.text.isNotEmpty) {
      ...
      final ok = pwd.length >= 8 &&
          RegExp(r'[A-Z]').hasMatch(pwd) &&
          RegExp(r'[0-9]').hasMatch(pwd) &&
          RegExp(r'[!@#$%^&*()\-_=+{};:,<.>?/\\|\[\]~`]').hasMatch(pwd);
      if (!ok) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('New password must be 8+ chars with 1 uppercase, 1 number, 1 special character'),
          backgroundColor: AppColors.danger,
        ));
        return;
      }
  ...
  Future<void> _saveBilling() async {
    final missing = _billingRequired.where((k) => _billing[k]!.text.trim().isEmpty).toList();
    if (missing.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Please fill all required fields (marked *) before saving'),
        backgroundColor: AppColors.danger,
      ));
      return;
    }
    ...
    try {
      await _api.dio.patch('/org/me/billing', data: {
        for (final k in _billing.keys) k: _billing[k]!.text.trim(),
      });
      if (mounted) await context.read<AuthState>().refresh();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  Future<void> _toggleAutoAssign() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(ApiClient.errorMessage(e, 'Failed to update setting')),
          backgroundColor: AppColors.danger,
        ));
      }
    } finally {
  ...
  Future<void> _loadAccessLog() async {
    ...
    try {
      ...
      setState(() => _accessRecords = (res.data['records'] as List? ?? []).cast<Map<String, dynamic>>());
    } catch (_) {
    } finally {
  ...
  Future<void> _respondAccess(String id, String action) async {
    try {
      await _api.dio.post('/org/support-access/$id/respond', data: {'action': action});
      _loadAccessLog();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  ImageProvider _imageProvider(String uri) {
    if (uri.startsWith('http://') || uri.startsWith('https://')) return NetworkImage(uri);
    final b64 = uri.contains(',') ? uri.split(',').last : uri;
    return MemoryImage(base64Decode(b64));
  }

  Widget _avatarPicker({required String? preview, required String fallback, required VoidCallback onPick, VoidCallback? onRemove}) {
    return Row(
      children: [
        Stack(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                image: preview != null && preview.isNotEmpty ? DecorationImage(image: _imageProvider(preview), fit: BoxFit.cover) : null,
              ),
              child: preview == null || preview.isEmpty
                  ? Center(child: Text(fallback, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: AppColors.primary)))
                  : null,
            ),
            Positioned(
              right: -4, bottom: -4,
          ...
          child: SizedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                if (isAdmin) _tabChip('organization', 'Organization', Icons.business_outlined),
                if (isAdmin) _tabChip('security', 'Security', Icons.lock_outline),
              ],
            ),
          ),
        ),
  ...
  Widget _profileTab(AuthState auth) {
    return ListView(
      ...
      children: [
        _avatarPicker(
          preview: _avatar,
          fallback: (auth.user?['name'] as String? ?? '?').isNotEmpty ? (auth.user!['name'] as String)[0].toUpperCase() : '?',
          onPick: _pickAvatar,
          onRemove: () => setState(() => _avatar = ''),
        ),
        const SizedBox(height: 8),
        Text(_name.text.isEmpty ? (auth.user?['name'] as String? ?? '') : _name.text, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
        Text(auth.user?['email'] as String? ?? '', style: const TextStyle(fontSize: 12, color: Colors.grey)),
        Container(
          ...
          child: Text(auth.role, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.primary)),
        ),
  ...
  Widget _billingField(String key, String label, {bool mono = false, bool numbers = false}) {
    ...
    return Padding(
      ...
      child: TextField(
        ...
        textCapitalization: numbers ? TextCapitalization.characters : TextCapitalization.none,
        decoration: InputDecoration(
  ...
  Widget _organizationTab() {
    final filledCount = _billingRequired.where((k) => _billing[k]!.text.trim().isNotEmpty).length;
    ...
    return ListView(
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: Text('Organisation & Billing Details', style: Theme.of(context).textTheme.titleSmall),
            ),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              decoration: BoxDecoration(
                color: (isComplete ? AppColors.success : AppColors.warning).withValues(alpha: 0.12),
                borderRadius: BorderRadius.circular(999),
              ),
              child: Text(
                isComplete ? 'Complete' : '$filledCount/${_billingRequired.length} required',
                style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: isComplete ? AppColors.success : AppColors.warning),
              ),
            ),
          ],
        ),
        const SizedBox(height: 4),
        Text('These details appear on every brokerage invoice sent to developers.',
        ...
        Text('ORGANISATION LOGO', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.grey.shade600, letterSpacing: 0.5)),
        const SizedBox(height: 8),
        _avatarPicker(preview: _logo, fallback: 'A', onPick: _pickLogo, onRemove: _logo != null && _logo!.isNotEmpty ? _removeLogo : null),
        const SizedBox(height: 20),
        Text('ORGANISATION IDENTITY', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.grey.shade600, letterSpacing: 0.5)),
        const SizedBox(height: 8),
        _billingField('address', 'Registered Address'),
        _billingField('phone', 'Contact Number'),
        _billingField('email', 'Official Email'),
        const SizedBox(height: 8),
        Text('TAX & COMPLIANCE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.grey.shade600, letterSpacing: 0.5)),
        const SizedBox(height: 8),
        ...
        const SizedBox(height: 8),
        Text('BANK DETAILS', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Colors.grey.shade600, letterSpacing: 0.5)),
        const SizedBox(height: 8),
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  const Text('Auto Lead Assignment', style: TextStyle(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 4),
                  Text(
                  ...
                  const SizedBox(height: 4),
                  Text(
                    _autoAssign ? 'Enabled — leads auto-assign on creation' : 'Disabled — assign manually',
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: _autoAssign ? AppColors.primary : Colors.grey),
                  ),
                ],
              ),
            ),
            _togglingAutoAssign
                ...
                : Switch(value: _autoAssign, onChanged: (_) => _toggleAutoAssign()),
          ],
        ),
        const SizedBox(height: 24),
      ],
  ...
  Widget _securityTab() {
    final pending = _accessRecords.where((r) => r['status'] == 'pending').toList();
    const reasonLabels = {
      ...
      child: ListView(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Support Access Log', style: Theme.of(context).textTheme.titleSmall),
                    Text('Track when Arthaleads support accesses your account', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
                  ],
                ),
              ),
            ],
          ...
          if (pending.isNotEmpty) ...[
            Text('PENDING APPROVAL', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.warning, letterSpacing: 0.5)),
            const SizedBox(height: 8),
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('${r['requestedByName'] ?? 'Someone'} is requesting access', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                      ...
                      Row(
                        ...
                        children: [
                          OutlinedButton(
                            onPressed: () => _respondAccess(r['_id'] as String, 'deny'),
                            style: OutlinedButton.styleFrom(foregroundColor: AppColors.danger),
                            child: const Text('Deny', style: TextStyle(fontSize: 12)),
                          ),
                          const SizedBox(width: 8),
                          ElevatedButton(
                            onPressed: () => _respondAccess(r['_id'] as String, 'approve'),
          ...
          if (_accessRecords.isEmpty)
            Padding(
              ...
              child: Column(
                children: [
                  Icon(Icons.verified_user_outlined, size: 40, color: AppColors.success.withValues(alpha: 0.3)),
                  const SizedBox(height: 8),
                  const Text('No support access on record', style: TextStyle(fontWeight: FontWeight.w600)),
                ],
              ),
            )
          else
            for (final r in _accessRecords)
              Card(
                ...
                child: ListTile(
                  ...
                  title: Row(
                    children: [
                      Text(r['requestedByName'] as String? ?? '—', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                      const SizedBox(width: 6),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 1),
                        decoration: BoxDecoration(
                          color: (statusColors[r['status']] ?? Colors.grey).withValues(alpha: 0.12),
                          borderRadius: BorderRadius.circular(999),
                        ),
                        child: Text(r['status'] as String? ?? '', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: statusColors[r['status']] ?? Colors.grey)),
                      ),
                    ],
                  ),
                  subtitle: Text('${reasonLabels[r['reason']] ?? r['reason'] ?? ''}${(r['notes'] as String? ?? '').isNotEmpty ? " — ${r['notes']}" : ""}',
                      style: const TextStyle(fontSize: 11)),
                  trailing: r['status'] == 'active'
                      ? TextButton(onPressed: _endAccessSession, child: const Text('End', style: TextStyle(fontSize: 11)))
                      : null,
                ),
              ),
        ],
