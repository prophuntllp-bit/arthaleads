...
class _TasksScreenState extends State<TasksScreen> {
  ...
  List<Map<String, dynamic>> get _tasks {
    ...
    return _allTasks.where((t) {
      ...
      if (_activeCard == 'today') {
        return !due.isBefore(todayStart) && !due.isAfter(todayEnd);
      }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        _allTasks = (results[0].data['tasks'] as List? ?? [])
            .cast<Map<String, dynamic>>();
        _summary =
            (results[0].data['summary'] as Map?)?.cast<String, dynamic>() ?? {};
        if (results.length > 1) {
          _agents = (results[1].data['agents'] as List? ?? [])
              .cast<Map<String, dynamic>>();
        }
        if (results.length > 2) {
          _projects = (results[2].data['data'] as List? ?? [])
              .cast<Map<String, dynamic>>();
        }
      });
    } catch (e) {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Marking "${task['title']}" as completed.',
  ...
  Future<Map<String, dynamic>?> _searchLead(String query) async {
    ...
    try {
      final res = await _api.dio.get(
        '/leads',
        queryParameters: {'search': query.trim(), 'limit': 8},
      );
      return {
        'leads': (res.data['leads'] as List? ?? [])
            .cast<Map<String, dynamic>>(),
      };
    } catch (_) {
      return null;
                    ...
                    debounce = Timer(
                      ...
                      () async {
                        ...
                        if (res != null) {
                          setSheetState(
                            () => leadResults =
                                res['leads'] as List<Map<String, dynamic>>,
                          );
                        }
                      },
                    );
      ...
      body: Column(
        children: [
          PageHeader(
            ...
            subtitle:
                '${_summary['today'] ?? 0} due today · ${_summary['overdue'] ?? 0} overdue',
